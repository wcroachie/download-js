...
window[new Error().stack.match(location.href.match(/(.*)\//g)+"(.*?):")[1]]=()=>{
  ...
  // look up table for file extensions and mimetypes
  var LUT={    
  ...
  var keys=Object.keys(LUT);
  var vals=Object.values(LUT);
  
  var isKey=(str)=>(keys.indexOf(str)===-1)?false:true;
  ...
  function fileExtensionToMimeType(ext){
    ...
    var check=isKey(ext);
    if(!check){
      console.warn("no file extension found for \""+ext+"\"");
      res="";
    }else{
      res=LUT[ext];
    }
  ...
  function mimeTypeToFileExtension(str){
    ...
    var check=isVal(str);
    if(!check){
      console.warn("no file extension found for \""+str+"\"");
      res="";
    }else{
